...
//size:內存大小(字節)
//返回值:分配到的內存首地址.
void *mymalloc_fix0(uint32_t size)
{
    uint32_t offset;
...
void *myrealloc_fix0(void *ptr, uint32_t size)
{
    uint32_t offset;
    offset = my_mem_malloc(0, size);
    ...
    {
        mymemcpy((void *)((uint32_t)mallco_dev.membase[0] + offset), ptr, size); //拷貝舊內存內容到新內存
        myfree(0, ptr);                                                  //釋放舊內存
        return (void *)((uint32_t)mallco_dev.membase[0] + offset);               //返回新內存首地址
    }
}
